...
{
    const QString devicePath = partition->devicePath();
    if ( devicePath.isEmpty() )
    {
        return Calamares::JobResult::error(
            QCoreApplication::translate( CreatePartitionJob::staticMetaObject.className(), "Failed to create partition" ),
            QCoreApplication::translate( CreatePartitionJob::staticMetaObject.className(),
                                         "Cannot create a ZFS partition without a device path." ) );
    }

    auto r = Calamares::System::instance()->runCommand(
        Calamares::System::RunLocation::RunInHost,
        { "sfdisk", "--append", "--force", devicePath },
        QString(),
        QStringLiteral( "start=%1 size=%2\n" ).arg( partition->firstSector() ).arg( partition->length() ),
    ...
    QString deviceNode;
    {
        ...
        if ( !rem.hasMatch() )
        {
            return Calamares::JobResult::error(
                QCoreApplication::translate( CreatePartitionJob::staticMetaObject.className(), "Failed to create partition" ),
                QCoreApplication::translate( CreatePartitionJob::staticMetaObject.className(),
                                             "sfdisk did not report the new ZFS partition." ) );
        }

        if ( rem.hasMatch() )
        {
            if ( devicePath.back().isDigit() )
            {
                deviceNode = devicePath + QLatin1Char( 'p' ) + rem.captured( 1 );
            }
            else
            {
                deviceNode = devicePath + rem.captured( 1 );
            }
        }
    ...
    if ( device->partitionTable()->type() == PartitionTable::gpt && partition->uuid().isEmpty() )
    {
        r = Calamares::System::instance()->runCommand(
            { "sfdisk", "--list", "--output", "Device,UUID", devicePath }, std::chrono::seconds( 5 ) );
        if ( r.getExitCode() == 0 )
        {
            QRegularExpression re( QRegularExpression::escape( deviceNode ) + QStringLiteral( " +(.+)" ) );
            QRegularExpressionMatch rem = re.match( r.getOutput() );
