...
static bool
isDeviceOrPartitionName( const QString& candidate, const QString& deviceName )
{
    if ( !candidate.startsWith( deviceName ) )
    {
        return candidate == deviceName;
    }

    const QString suffix = candidate.mid( deviceName.size() );
    return suffix.isEmpty()
        || ( !suffix.isEmpty() && suffix.at( 0 ).isDigit() )
        || ( suffix.startsWith( QStringLiteral( "p" ) ) && suffix.size() > 1 && suffix.at( 1 ).isDigit() )
        || suffix.startsWith( QStringLiteral( "boot" ) )
        || suffix.startsWith( QStringLiteral( "rpmb" ) );
}
...
getPartitionsForDevice( const QString& deviceName )
{
    ...
    if ( dev_partitions.open( QFile::ReadOnly ) )
    {
        ...
        while ( !in.atEnd() )
        {
            ...
            if ( ( columns.count() >= 4 ) && isDeviceOrPartitionName( columns[ 3 ], deviceName )
    ...
    if ( process.exitCode() == 0 )  //means LVM2 tools are installed
    {
        ...
        if ( !pvdisplayOutput.simplified().isEmpty() )  //means there is at least one LVM PV
        {
            ...
            for ( const QString& pvdisplayLine : pvdisplayLines )
            {
                ...
                const QString pvName = pvPath.section( '/', -1 );
                if ( !isDeviceOrPartitionName( pvName, deviceName ) )
                {
                    continue;
                }
